;(function($,jQuery){
    ...
    $.fn.banner = function(options){
        ...
        this._obj.init = function(){
            ...
            that.html(`<div class="imgbox container">${str}</div>`).css({
                width:1200,
                height:470,
                position:"relative",
                overflow:"hidden"
        ...
        this._obj.btnMove = function(type){
            ...
            if(!this.list) return;
            that.children(".list").children("li").css("background","rgba(200,200,200,0.8)").eq(this.index).css("background","red");
        }
        // 3.判断用户是否需要按钮功能
        if(this._obj.btn){
            // 左右按钮的布局+样式
        ...
        if(this._obj.list){
            ...
            this._obj.listMove = function(iNow,type){
                ...
                that.children(".list").children("li").css("background","rgba(200,200,200,0.8)").eq(iNow).css("background","red");
            }
        }
